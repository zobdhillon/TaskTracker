            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('tasks.update', ['task' => $task['id']]) }}"
                        ...
                        <x-form.category-select :categories="$categories" :selected="$task['category']['id'] ?? null" />

                        <!-- Task Date -->
                        <div class="mt-4">
                            <x-input-label for="task_date" :value="__('Task Date')" />
                            <x-text-input id="task_date" class="block mt-1 w-full" type="date" name="task_date"
                                :value="old('task_date', $task['task_date']['display'])" />
